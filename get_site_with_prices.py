...
def run_site_crawl(db_path, sites, preferences, dump_location):
    ...
    """for site in sites:
        ...
        for i in range(0, num_browser):
			manager.get(site,i)"""
    for site in sites:
        manager.browse(site,2, None, 60000)
        saved +=1
        if saved == 10:
			saved = 0
			save_n += 1
			if dump_location:
				for browser_index in range(0, num_browser):
					manager.dump_profile(dump_location+"/profile_"+str(browser_index)+ "_" + str(save_n),False,browser_index,1200000)
        
    if dump_location:
		for browser_index in range(0, num_browser):
			manager.dump_profile(dump_location+"/profile_"+str(browser_index),False,browser_index,1200000)
