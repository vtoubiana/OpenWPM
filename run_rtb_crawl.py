...
def do_exp(age, s, interest, shop, browser_index):
	...
	for site in g_sites[interest]:
		manager.get(site,browser_index)
	if shop :
		manager.buying_intent(interest,browser_index)
	time.sleep(randint(10,100))
	"""for x in range(0, 1):
		manager.get("http://www.binnews.me/index.php?country=fr",browser_index)
		manager.get("http://www.calciomercato.com/",browser_index)
		manager.get("http://99covers.com/",browser_index)"""
	dump_location = "profiles6/profile_"+s+"_"+str(age)+"_"+interest+"_"+str(shop)
...
def run_site_crawl(db_path, g_sites, preferences, dump_location):
    """
		...
		for age in [20,30,40]:
			for interest in ["sport","shopping","computers"]:
				...
				for shop in [True,False]:
					...
					tasks[browser_index].append([age, s, interest, shop])
					index +=1
    for i in range(0,num_browser):
...
def main(argv):
    ...
    run_site_crawl(db_path, g_sites, preferences, dump_location)
