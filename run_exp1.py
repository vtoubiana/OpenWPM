...
def do_exp(age, s, interest, shop, dnt, browser_index):
	...
	dump_location = "experimentation4/profile_"+s+"_"+str(age)+"_"+interest+"_shop_"+str(shop)+"_dnt_"+str(dnt)
...
def do_serialized_exp(params):
	...
	for task in tasks:
		...
		do_exp(task[0],task[1],task[2],task[3], task[4], browser_index)
		...
		for age in [20,30,40]:
			for interest in ["sport","shopping","computers"]:
				...
				for shop in [True,False]:
					...
					tasks[browser_index].append([age, s, interest, shop, dnt])
					index +=1
    for i in range(0,num_browser):
