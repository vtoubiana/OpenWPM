...
class TaskManager:
    ...
    def _save_configuration(self, browser_params):
        ...
        cur.execute(("INSERT INTO task "
                     ...
                     "VALUES (?,?,?)"),
                (json.dumps(self.manager_params), 0, 0))
        self.db.commit()
    ...
    def _distribute_command(self, command, index=None, timeout=60, reset=False):
        """
    ...
    def buying_intent(self, shope="sport", index=None, timeout=60, reset=False):
        """ Buying Intent """
        print "Will call command buy intent"
        self._distribute_command(('BUYING_INTENT', shope), index, timeout, reset)
    
    def exelate_settings(self, age=None, gender=None, index=None, timeout=60, reset=False):
        """ set google ad settings """
        print "Will call command set setting on browser "+ str(index)
        self._distribute_command(('EXELATE_SETTINGS', age, gender, None, None), index, timeout, reset)
      
    def google_settings(self, optin=True, index=None, timeout=60, reset=False):
        """ set google ad settings """
        print "Will call command set setting"
        self._distribute_command(('GOOGLE_SETTINGS', optin), index, timeout, reset)

    def delete_all_cookies(self, index=None, timeout=60, reset=True):
        """ goes to a url """
        print "Will call command delete cookies on" +str(index)
        self._distribute_command(('DELETE_COOKIES',), index, timeout, reset)
        
    def opt_all_out(self, index=None, timeout=60, reset=False):
        """ goes to a url """
        print "Will call command opt-out on" +str(index)
        self._distribute_command(('OPT_ALL_OUT',), index, timeout, reset)
    ...
    def extract_links(self, index = None, timeout=60):
        self._distribute_command(('EXTRACT_LINKS',), index, timeout)
        
    def load_profile(self, load_tar, index=None, timeout=60):
        """ Load a from the tar to the current profile folder (absolute path) """
        self._distribute_command(('LOAD_PROF', load_tar), index, timeout)
